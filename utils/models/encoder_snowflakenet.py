...
class HyperSnowFlakeNetEncoder(nn.Module):
    def __init__(self, bottleneck=1024):
        """Hyper Encoder that encodes information of partial point cloud. It
        differs from SnowFlakeNetEncoder in using batch norm layers in SAModule.
        """
        super(HyperSnowFlakeNetEncoder, self).__init__()
        self.sa_module_1 = PointNet_SA_Module_KNN_WithBN(512, 16, 3, [64, 128], group_all=False, if_bn=True, if_idx=True)
        self.transformer_1 = Transformer(128, dim=64)
        self.sa_module_2 = PointNet_SA_Module_KNN_WithBN(128, 16, 128, [128, 256], group_all=False, if_bn=True, if_idx=True)
        self.transformer_2 = Transformer(256, dim=64)
        # self.sa_module_3 = PointNet_SA_Module_KNN_WithBN(None, None, 256, [512, bottleneck], group_all=True, if_bn=True, is_print=True)
        self.sa_module_3 = PointNet_SA_Module_KNN_WithBN(None, None, 256, [512, bottleneck], group_all=True, if_bn=True, is_print=False)
    ...
    def forward(self, x, pos, batch):
        ...
        Args:
             ...
             batch: [N]
        Returns:
            l3_points: (B, bottleneck)
        """
        batch_size = batch.max() + 1
        point_cloud = pos.view(batch_size, -1, 3).transpose(1, 2).contiguous() # (B,3,n)
        ...
        l3_points = l3_points.squeeze(dim=-1)

        return l3_points


class SnowFlakeNetEncoder(nn.Module):
    def __init__(self, bottleneck=1024):
        """Encoder that encodes information of partial point cloud
        """
        super(SnowFlakeNetEncoder, self).__init__()
        self.sa_module_1 = PointNet_SA_Module_KNN(512, 16, 3, [64, 128], group_all=False, if_bn=False, if_idx=True)
        self.transformer_1 = Transformer(128, dim=64)
        self.sa_module_2 = PointNet_SA_Module_KNN(128, 16, 128, [128, 256], group_all=False, if_bn=False, if_idx=True)
        self.transformer_2 = Transformer(256, dim=64)
        self.sa_module_3 = PointNet_SA_Module_KNN(None, None, 256, [512, bottleneck], group_all=True, if_bn=False)
...
class PointNet_SA_Module_KNN_WithBN(nn.Module):
    def __init__(self, npoint, nsample, in_channel, mlp, if_bn=True,
            group_all=False, use_xyz=True, if_idx=False, is_print=False):
        """
        Args:
            npoint: int, number of points to sample
            nsample: int, number of points in each local region
            ...
            mlp: list of int,
        """
        super(PointNet_SA_Module_KNN_WithBN, self).__init__()
        self.print = is_print
        self.npoint = npoint
        self.nsample = nsample
        self.mlp = mlp
    ...
    def forward(self, xyz, points, idx=None):
        ...
        if self.print:
            for layer in self.mlp_conv:
                ...
                print(new_points.size(), new_points.norm(dim=1).mean().item())
        else:
            new_points = self.mlp_conv(new_points)
